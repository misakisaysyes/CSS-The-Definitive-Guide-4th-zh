...
This will also be the style used as a fallback in any system where a counter can’t be represented by the primary system, for whatever reason. A simple example is a count‐
ing system that uses images for its symbols, and one of the images fails to load. In the following, assume south.svg fails to load. In that case, the missing image would be
...
symbol is treated as a digit in a numbering system. If you’ve spent any time in spread‐
sheets, this counting method may be familiar to you from the column labels.
...
switching over to “BA” through “BE”, then on to “CA” and so on. In the symbolic ver‐
sion of this, we’d already be up to “EEEEEE” by the time “EE” was reached in the
alphabetic system.
...
sign (-), but what about in other systems, like symbolic? What if we define a letterbased numeric counting system? Or if we want to use accounting-style formatting,
...
Also note that the negative sign will come outside any padding. Thus, given the fol‐
lowing styles, we’d get the result shown in Figure 15-43:
...
number of the symbols together properly, and adding up the numbers each repre‐
sents, yields the counter value.
...
the symbols descriptor will render the entire counter-styles block invalid. (Simi‐
larly, attempting to use the additive-symbols description in non-additive systems
will render those blocks invalid.)
One last thing to note about additive systems is that, due to the way the additivecounter algorithm is defined, it’s possible to create additive systems where some val‐
ues can’t be represented even though it seems like they should be. Consider this
definition:
...
this is required to maintain the property that the algorithm runs in linear time rela‐
tive to the size of the counter value.”
...
counting algorithm. Even if that doesn’t interest you, just remem‐
ber: make sure you have a symbol whose value equates to 1, and
...
For example, suppose you want to change regular decimal counting to use closeparentheses symbols as suffixes, and pad up to two leading zeroes. You could write it
...
```
That takes the existing decimal system familiar from list-style-type and refor‐
mats it a bit. Thus, there’s no need to re-type the whole symbol chain. You just adjust
the options, as it were.

In fact, you can only adjust the options: if you try use either symbols or additivesymbols in an extneds system, the entire @counter-style block will be invalid and
...
However, you can vary the hexadecimal counting for different contexts. As an exam‐
ple, you could set up basic hex counting and then define some variant display pat‐
terns, as shown in the following code and illustrated in Figure 15-45. (Note that each
...
hexcolon, which itself extends the first, hexadecimal. In hexcolonlimited, the hexa‐
decimal counting stops at FF (255), thanks to the range: 1 255; declaration.

### 15.3.8 Speaking Counting Patterns

While it’s fun to build counters out of symbols, the result can be a real mess for spo‐
ken technologies such as Apple’s VoiceOver or the JAWS screen reader. Imagine, for
...
might provide an audio fallback for dice-counting to be decimal counting when spo‐
ken:
```css
@counter-style dice {
 system: additive;
...
With the value numbers, the counters are spoken as numbers in the document lan‐
guage. This is very similar to the previous code sample, where “⚅⚅⚂” is spoken as
...
effect depends on the counting system in use. If it’s a alphabetic system, then speak816 | Chapter 15: Lists and Generated Content
...
effects are determined based on the system being extended. Therefore, given the fol‐
lowing styles, the counters in an emojibrackets list will be spoken as if speak-as
were set to bullets:
```css
...
remove the markers and indentation, and thus create a navigation sidebar. The com‐
bination of simple markup and flexible layout is difficult to resist. With the anticipa‐
ted enhancements to list styling in CSS3, I expect that lists will become more and
more useful.
...
Even better, thanks to counters, you can now associate ordering information to ele‐
ments that are not typically lists, such as headings or code blocks. Now, if you want to
