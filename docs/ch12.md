...
> Flexbox 可以脱离父子关系。通过声明 display: flex 或 display: inline-flex 在元素上激活 Flexbox 布局。该元素成为一个 flex 容器，将其子元素布置在提供的空间内并控制其布局。此伸缩容器的子 flex 项成为项。考虑以下样式和标记，如图 12-1 所示：
...
> 注意 divs 的每个子元素如何变成弹性项目，此外，它们如何以相同的方式布置？某些是段落而其他是 spans 没关系。他们都成为弹性物品。（由于删除了这些段落，因此由于段落的空白，可能会有一些差异。）
...
> 第一和第二个 flex 容器之间的唯一真正的区别是一个被设置为 display: flex，另一个被设置为 display: inline-flex。在第一个中，div 变成一个内部具有 Flex 布局的块盒。在第二个中，div 变成一个内联块框，其中包含 flex。
...
> 要牢记的关键是，一旦将一个元素设置为 flex 容器（div 如图 12-1 中的 s），它将仅显示其直接子代，而不显示其后代。但是，您也可以使这些子代伸缩容器，从而启用一些非常复杂的布局。
...
> 在 flex 容器中，项目在上排列 main axis。主轴可以是水平或垂直的，因此您可以将项目排列为列或行。主轴采用通过书写模式设置的方向性：稍后将深入讨论该主轴概念（请参见第 579 页的“理解轴”）。
...
> div 如图 12-1 中的第一个示例所示，当 flex 项目没有填满容器的整个主轴（在这种情况下为宽度）时，它们将留下额外的空间。有一些属性指示如何处理额外的空间，我们将在本章的后面部分进行探讨。您可以将孩子分组到左侧，右侧或居中，也可以将它们散开，定义在孩子之间或周围散布空间的方式。
...
> 除了分配空间外，还可以通过在一个，某些或所有弹性项目之间分配额外的空间，使弹性项目增长以占用所有可用空间。如果没有足够的空间来容纳所有弹性项目，则可以使用弹性框属性来指示如何收缩它们以适合其容器，或者是否允许它们包装到多条弹性线。
...
> 此外，孩子们可以相对于他们的容器或彼此对齐。到容器的底部，顶部或中心；或拉长以填充容器。不管兄弟容器之间内容长度的不同，使用 flexbox 都可以通过一个声明使所有兄弟具有相同的大小。
...
> 在前面的代码中，其 display 属性设置为 flex，该 nav 元素被转换为 flex 容器，并且其子链接均为 flex 项。这些链接仍然是超链接，但就其表示而言，它们也是灵活的项目。它们不再是内联级别的框：相反，它们参与了容器的 flex 格式上下文。因此，就 a 布局而言，元素之间的空白将被完全忽略。如果您曾经使用 HTML 注释来隐藏链接，列表项或其他元素之间的空格，那么您就会知道为什么这么重要。
...
> 现在这似乎还不算什么，因为这里没有什么可以用老式 CSS 来完成的。稍等：它会变得更好。
...
> 根据设计，flexbox 与方向无关。这与块或行内布局不同，后者分别定义为垂直和水平偏置。该网络最初是为在监视器上创建页面而设计的，并且假定水平约束具有无限的垂直滚动。这种垂直偏置的布局不足以根据用户代理和视口的方向更改方向，增大和缩小以及根据语言更改书写方式的现代应用程序。
...
> 多年来，我们开玩笑说垂直居中和多列布局面临挑战。某些布局绝非易事，例如确保在多个并排的框的网格中具有相等的高度，将按钮或“更多”链接固定在每个框的底部，并且将按钮的内容整齐地垂直居中，如图所示。图 12-3; 或者，确保内容库中所有框的高度都相同，而顶部画廊行的框与后续行中的框整齐地对齐，如图 12-4 所示；或者，使单个按钮的各个部分整齐地排列，如图 12-5 所示。Flexbox 使所有这些挑战变得相当简单。
...
> 大多数设计要求相等的高度的列，但增加的背景的 aside，article 以及 nav 会放大，他们有不同的高度。为了提供等高列的外观，我们经常根据 CSS 中声明的列宽向父级添加虚假背景，使用大量填充和负边距，插入清除的生成内容，以及其他技巧。
...
> 由于所有这些技巧使我们的 CSS（以及我们的 HTML）杂乱无章，旧的布局方法可能会造成混乱。许多人开始使用 YUI 网格，Bootstrap，Foundation，960 网格和其他 CSS 布局库，只是为了给他们的开发过程带来一点点理智。希望这本书可以帮助您认识到不再需要 CSS 框架来保持布局样式的健全。
...
> 这些子 flex 项是项，无论它们是 DOM 节点，文本节点还是生成的内容。弹性容器的绝对定位子元素也是弹性项目，但是每个子元素的大小和位置都好像是弹性容器中唯一的弹性项目。我们将首先了解适用于 flex 容器的 CSS 属性，包括一些影响 flex 项布局的属性。弹性项目本身是您需要了解的主要概念，稍后将在第 609 页的“弹性项目”中全面介绍。
...
> display 图 12-1 中的属性示例在一行上并排显示了三个 flex 项目，从左到右。通过一些附加的属性值声明，我们可以使项目居中，将它们与容器的底部对齐，重新排列其外观顺序，或从左至右或从上至下进行布局。我们甚至可以使它们跨越几行。
...
> 有时我们会有一个 flex 项目，有时我们会有几十个。有时我们会知道一个节点有多少个孩子，有时孩子的数量将不受我们的控制。我们可能知道项目的数量，但不知道容器的宽度。我们应该有健壮的 CSS，当我们不知道会有多少个 flex 项目或 flex 容器有多宽时（考虑响应），就可以处理我们的布局。幸运的是，flexbox 使得这一切比听起来容易得多，并且只需几个新属性即可做到。
...
> 如果要使布局从上到下，从左到右，从右到左，甚至从下到上，则可以使用 flex-direction 来控制弹性项目沿其布置的主轴。
...
> 该 flex-direction 属性指定将弹性项目放置在弹性容器中的方式。它定义了伸缩容器的主轴，该主轴是伸缩项目沿其布置的主轴（有关更多详细信息，请参见第 579 页的“理解轴”）。
...
> 图 12-6 显示了 flex-direction 假设从左到右的语言，如何通过 apply 的四个值中的每个来排列该简单列表。
...
> 默认值 row 看起来与一堆内联或浮动元素没有什么不同。由于我们很快会看到的原因，这具有误导性，但是请注意其他 flex-direction 值如何影响列表项的排列。
...
> 例如，您可以使用反转项目的这种布局 flex-direction: row-reverse。设置时，flex 项目从上到下布置，如果 flex-direction: column 设置，则从下到上布置 flex-direction: column-reverse，如图 12-6 所示。
...
> 我们指定了从左到右的语言，因为主轴 row 的方向（即弹性项目的放置方向）是当前书写模式的方向。我们将稍微讨论写入模式如何影响柔韧性方向和布局。
...
> 因此，在声明 column 方向时，弹性项目的显示顺序与源文档中声明的顺序相同，但从上到下而不是从左到右显示，因此，弹性项目在下一个而不是侧面的顶部布置并排。考虑：
...
> 使用这样的标记，只需更改一些 CSS 属性，我们就可以为我们之前看到的作为选项卡的水平行的链接列表创建一个漂亮的边栏式导航。对于新布局，我们只需将其 flex-direction 从默认值更改 row 为 column，将边框从底部移至右侧，然后更改颜色 border-radius，和 margin 值，结果如图 12-7 所示。
...
> 该 column-reverse 值与相似 column，不同之处在于主轴反转，主轴起点在垂直主轴的底部，主轴终点在垂直主轴的顶部，如图 12-6 所示。反向值只会更改外观。语音顺序和制表符顺序与基础标记相同。
...
<body>
  ...
  <main>
    ...
    <article>
      ...
      <p>
        We have lots of content here to show that content can grow, and everything can be the same size if you use
        flexbox.
      </p>
...
```html
<p style="display: flex;"><strong>Flex items:</strong> they’re what’s for <em>&lt;br&gt;fast!</em></p>
```
