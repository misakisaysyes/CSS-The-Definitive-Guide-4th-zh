...
> Flexbox 是一种通过指示空间分布方式，内容对齐方式和视觉元素排序方式来布局页面组件的简单而强大的方法。内容可以轻松地垂直或水平排列，并且可以沿单个轴布局或跨多行包装。还有更多。
...
> 除了分配空间外，还可以通过在一个，某些或所有 flex 子元素之间分配额外的空间，使 flex 子元素增长以占用所有可用空间。如果没有足够的空间来容纳所有 flex 子元素，则可以使用弹性框属性来指示如何收缩它们以适合其容器，或者是否允许它们折行到多个 flex 行。
...
> 这些子元素是 flex 项，无论它们是 DOM 节点，文本节点还是生成的内容。弹性容器的绝对定位子元素也是弹性项目，但是每个子元素的大小和位置都好像是弹性容器中唯一的弹性项目。我们将首先了解适用于 flex 容器的 CSS 属性，包括一些影响 flex 子元素布局的属性。弹性项目本身是您需要了解的一个主要概念，稍后将在第 609 页的“flex 子元素”中全面介绍。
...
> 该 flex-direction 属性指定将 flex 子元素放置在弹性容器中的方式。它定义了 flex 容器的主轴，该主轴是 flex 子元素沿其布置的主轴（有关更多详细信息，请参见第 579 页的“理解轴”）。
...
> 是的，元素既可以是 flex 容器，又可以是 flex 子元素，正如我们在本例中看到的导航，主要内容和文章。 正文和文章的弹性方向设置为`column`，我们将 nav 和 main 默认设置为`row`。 只需两行 CSS！
...
> 当设置为 `wrap-reverse` 时，交叉轴方向是颠倒的: 在 `row` 和 `row-reverse` 情况下，后续的线在顶部绘制，在 `column` 和 `column-reverse` 情况下，后续的线在前一列的左侧绘制。 同样，在从右到左的语言中，`row wrap-reverse`和`row-reverse wrap-reverse`也会在顶部添加新的行，但是对于 `column wrap-reverse` 和 `column-reverse wrap-reverse`则会在右侧添加新的行ーー与语言方向或写入方式相反的方向，即倒置的交叉轴的方向。
...
> 如果在 flex 容器上设置了“ flex-flow：column wrap”，则如果没有足够的空间容纳 flex 子元素到第一列中，它们将换行。下一个 flex 项目将在交叉轴方向上放置在新行上，在这种情况下，它是前一行右边的垂直行（一列），如在图 12-16 中的示例：“flex-flow：column wrap”。在那种特定情况下，弹性项目已折到三行上。当我们设置“ flex-flow：column-reverse”时，发生了相同的事情，交换了交叉起始和交叉末端的位置，因此初始列在右边，随后的列（flex lines）被添加到该初始列的左侧。
...
> “ justify-content”属性控制flex行中的flex子元素如何沿主轴分布。 “ align-content”定义了flex子元素沿交叉轴分布的方式。 “ align-items”属性定义了每个flex子元素的行在交叉轴方向怎么摆放。让我们从在行内布置flex子元素开始。
...
>使用`align-items`，您可以将所有子元素设置为与交叉轴起点或交叉轴终点齐平，或拉伸至两者都齐平。或者，您可以将所有flex子元素在flex行的中间居中。有五个值，包括` flex-start`，`flex-end`，`center`，`baseline`和默认的`stretch`，如图12-29所示。
...
>在图12-29中，请注意flex子元素如何对齐flex容器的交叉轴起点或交叉轴终点，居中或拉伸以对其两者（除了`baseline`以外）。使用`baseline`，可调整flex子元素的基线：在基线和交叉起点之间具有最大距离的flex子元素将与交叉轴起点齐平。
...
>对于每行，红线是交叉轴起点，蓝色是交叉轴终点，当折行的行后接了新行后，这些线将显示为紫色。 C，H，D和I的顶部和底部边距值不同。我们在所有flex子元素的侧面增加了一些边距，以使图形更清晰易读，在这种情况下，这不会影响“ align-items”属性的影响。 J的字体大小增加了，增加了行高。这将在我们讨论`baseline`值时发挥作用。
...
>否则，flex子元素的交叉轴起点将与flex行的交叉轴起点齐平，而flex子元素的交叉轴终点将与flex线的交叉终点齐平。在交叉轴方向尺寸最大的flex子元素将保留其默认大小，其他flex子元素将增大到该最大flex子元素的尺寸。
...
>它们的边距是C，D，H和I看起来比该行上的其他子元素小的原因。他们不是。上边距和下边距的外边缘与它们所占据的行的交叉起点和交叉端点齐平。这些行的高度与线上的最高子元素一样高，或者在交叉轴方向为水平时与最宽的项子元一样宽。
...
>flex行的高度或宽度仅与包含flex子元素所需的高度或宽度相同。在图中五个`align-items`里，仅包含K的flex行的高度比其他两行小得多。
...
>“ flex-start”值将每个flex子元素的交叉起始边缘与它们的行的交叉起始边缘对齐。flex子元素的交叉起始边缘在边距的外部：如果flex子元素的边距大于0，则flex子元素将不会与flex行的交叉起始边缘齐平，如在图12-32中第一个示例中的flex子元素C，D，H和I中所示。
...
>`baseline`值稍微复杂一些。使用`baseline`，每行中的flex子元素都在其第一个基线处对齐。每行的flex子元素的基线和交叉起始边缘之间的距离最大的子元素，该元素的交叉起点方向边缘与行的起始边缘齐平，所有其他flex子元素的基线都与该flex子元素基线对齐。
...
>看一下图12-33中的第二行，其中J占主导地位。 J中的字体大小已增加到“ 3rem”，以便创建比其他flex子元素具有更高的第一行文本的flex子元素。其顶部（交叉起点）边缘紧贴着行的顶部（交叉起点）边缘。我们向下移动该行中的所有其他flex子元素，直到其第一行文本的基线与J的第一个基线对齐为止。（绿线表示此基线的位置。）
...
>现在看第一行，以A开头。您会注意到A，B，C，D和E都是顶部对齐，但看起来更近。此处的微妙之处在于，它们显然没有与行的顶部齐平。发生这种情况是因为D的上边距为20像素。 D的顶部（交叉起点）外边距边缘与fle行的交叉起点齐平。如前所述，交叉起始行与基线之间的距离由该行上在其交叉起始侧上的外部边缘与基线之间具有最大距离的子元素确定。因此，D的位置（由于其顶部边距）成为该行中其他子元素对齐的基准。
...
>在许多情况下，`baseline`看起来像 `flex-start`。例如，如果D缺少上边距，那么第一行中的所有子元素都将明显地与flex行的顶部齐平，就像使用`flex-start`一样。只要这些子元素的交叉起始端的外边距，边框，内边距，字体大小或行高不同，·`flex-start`和`baseline`之间就会有所不同。
...
>在某些情况下，`baseline`从字面上变为`flex-start`，那就是flex子元素的基线平行于交叉轴。例如，假设我们使用了图12-33中的flex容器，并将其更改为`flex-direction：column`。现在，交叉轴像其中的英文文本的基线一样，是水平的。由于无法从列的交叉起始边缘（左侧）创建偏移，因此将 `baseline`视为完全是 `flex-start`。
...
>在`align-items`示例中，flex容器的交叉轴方向尺寸已达到所需的高度。容器上没有声明`height`，因此默认为` height：auto`。因此，flex容器逐渐适合内容。您可能已经注意到示例flex容器的高度都相同，并且所有示例的flex行高度都相同。
...
>如果将交叉轴方向尺寸（在这种情况下为高度）设置为特定尺寸，则交叉终点可能有多余的空间，或者没有足够的空间来容纳内容。 Flexbox允许我们使用`align-content`属性来控制行的对齐方式。 ` align-content`属性是我们需要关注的最后一个属性，它适用于flex容器（相对于flex子元素）。`align-content`属性仅影响多行flex容器中的flex行对齐。
...
>就像`align-items`属性的值一样，`flex-start`值将子元素放置在交叉起点边缘。 `flex-end`将子元素放置在交叉端边缘。`center` 在交叉轴的中间对齐子元素。 `baseline`将flex子元素的基准线与其flex行中的最低基准线对齐。最后，auto和stretch都拉伸了flex子元素，因为`align-items`值被默认设置为`stretch`。 （同样，在这种情况下，`align-self：Inherit`会导致拉伸对齐。）
...
>“ align-content”属性将伸缩容器中的线对齐，使其在横轴方向上具有额外的空间，并指示当没有足够的空间容纳伸缩线时，哪个方向会溢出。
...
>“ align-content”属性决定了flex容器中任何多余的横向空间如何在flex线之间和周围分布。尽管值和概念相同，但是“ align-content”与之前讨论的“ align-items”属性不同，后者规定了伸缩项目在每个伸缩行内的位置。
...
>可以将“ align-content”想像成与“ justify-content”如何沿flex容器的主轴对齐单个项目类似，但是它针对容器的横轴针对flex线进行对齐。此属性仅适用于多行伸缩容器，对非包装和单行伸缩容器没有影响。

Consider the following CSS as a base, and assume the flex items have no margins:
>以以下CSS为基础，并假设flex项目没有空白：
...
>图12-35演示了与CSS结合使用的“ align-content”属性的七个可能值。在每个示例中，都有三条柔性线。每条柔性线的交叉起点和交叉终点分别由红线和蓝线表示。 flex容器中的剩余空间；也就是说，柔性线之间或周围的空间由带状区域表示。
...
>flex容器的高度为480像素，比3条flex线的默认组合高度高。假设每行中最高的项目（E，F和K）分别为150像素，180像素和30像素，总共为360像素。每个伸缩容器在横向尺寸方向上都有额外的120像素的可用空间。
...
>如图12-35所示，通过五个“ align-items”值，自由空间分布在柔性线的外部。它们以相同的方式对“ justify-content”进行相同的操作，只是沿横轴而不是主轴（“ justify-content”就是这种情况）。值为“ stretch”时，多余的空间将均匀地分配到所有柔性线，从而增加其交叉尺寸，直到其边缘接触为止。
...
>在前面的示例中，弹性容器的高度为480像素，我们在横轴上有120像素的“剩余”空间，其分布取决于“ align-content”属性的值。
...
>如图12-35的前三个示例所示，使用“ flex-start”时，120个像素全部位于横轴的交叉端。使用“ flex-end”时，多余的120像素可用空间全部放在交叉起点。使用居中时，线居中，并且在交叉起点和交叉终点放置60像素的额外空间（一半为120像素）。
...
>在“ space-between”之间，相邻的一对柔性线之间有60个像素。另一方面，在“ space-around”中，空间均匀地分布在每行周围：由于有3条柔性线，因此120像素被分成3个像素。这会将20像素的非折叠空间（一半为40像素）放置在每条伸缩线的交叉起点和交叉端侧，因此伸缩容器的交叉起点和交叉端侧有20像素的额外空间，相邻的折线之间的间距为40像素。
...
>对于“平均间隔”，有四个空格要插入：在每个伸缩线之前一个空格，在最后一个伸缩线之后一个多余的空格。三行表示四个空间，每个空间30像素。这样在交叉起点和交叉终点放置了30个像素的空间，在相邻的柔性线之间放置了30个像素。
...
>`stretch'值是不同的：使用`stretch`时，线拉伸时，多余的空间均匀地分布在柔性线之间而不是在柔性线之间。在这种情况下，向每个柔性线添加了40个像素，从而使所有这3个柔性线的高度都增长了相等的数量，即完全相同的数量，而不是与每个像素成比例的数量。您会注意到，在图12-35的第六个示例中，容器内没有未被柔性线占据的区域。 “ stretch”是默认值，因为您可能想填充所有可用空间。
...
>如果没有足够的空间容纳所有行，则它们会在交叉起点，交叉终点或两者同时溢出，具体取决于`align-content`属性的值。如图12-36所示，其中带有浅灰色背景的虚线框表示一个短的弯曲容器。 （“ align-items：flex-start”的设置使“ align-content”的效果更加明显。）
...
>CSS与图12-35之间的CSS唯一区别是flex容器的高度。在这里，伸缩容器的高度已减小到240像素，以便创建足够高的伸缩容器以容纳其所有伸缩线（您可能还记得，它们的总高度为360像素）。
...
>当伸缩线溢出伸缩容器时，“ align-content：flex-start”，“ space-between”和“ stretch”会导致它们的两端溢出，而“ align-content：space-around”和“居中”会在交叉端和交叉起始端均匀溢出。只有`align-content：flex-end`会导致flex线仅在crossstart端溢出。
...
>请记住，这些值不是顶部或底部中心。如果十字轴朝上，那么`align-content：flex-start`将开始从底部开始对齐弯曲线，然后从底部开始向上弯曲，可能会溢出顶部（交叉端）边缘。因此，当流动方向为柱状时，交叉轴将为水平，在这种情况下，交叉起点和终点边缘将为flex容器的右边缘或左边缘。
...
>值得仔细研究一下，并思考“之间的空间”和“周围的空间”如何影响柔线的对齐方式。
...
>当设置了“ align-content：space-between”时，伸缩线均匀地分布在伸缩容器中。这种“均匀分布”是基于可用空间，而不是线的大小。如果有多个弯曲线，则第一行将与容器的交叉起点齐平，最后一行将与容器的交叉端点齐平，并且如果有的话，任何可用的额外空间将在附加线之间平均分配有。多余的空间均匀分布，而不是成比例分布。即使多个伸缩线的交叉尺寸不同，伸缩容器中任意两条伸缩线之间的间距也相等。此外，如果折线的数量奇数，则中间的折线不一定会在flex容器中居中，因为这些线不一定都具有相同的交叉尺寸。
...
>相反，任何两条相邻线之间的间距都相同。如上一节所述，假定3行共有120像素的可用空间。第一条柔性线靠在交叉起点边缘，第二条柔性线靠在交叉终点边缘。这意味着它们之间只有一条线，还有两个间隙。剩余空间的120像素被平均分为2个块​​，每个块60个像素。一个60像素的块放置在第一和第二弯曲线之间，另一个放置在第二和第三弯曲线之间。如图12-37所示。
...
>'space-around'值将线均匀地分布在多行伸缩容器中，就好像所有伸缩行在开始和结束两端都具有相等的，不塌陷的边距一样。由于每条线周围的可用空间均等分布，因此容器的边缘与第一条和最后一条柔性线之间的空间是任何两条柔性线之间距离的一半。多余空间的分布如图12-37所示。
...
>我们大部分时间都在研究flex容器的属性（“ align-self”是一个例外）。现在该看看直接应用于弹性商品的属性了。
...
>在前面的部分中，我们了解了如何通过样式化容器在Flex容器中全局布局所有flex项目。灵活框布局规范提供了一些直接适用于弹性项目的其他属性。通过这些flex-itemspecific属性，我们可以更精确地控制单个flex容器的子代的布局。
...
>我们只需在具有子节点的元素上添加“ display：flex”或“ display：inline-flex”即可创建flex容器。这些flex容器的子级称为“ flex项”，无论它们是子元素，子元素之间的非空文本节点还是生成的内容。图12-38显示了一种情况，其中每个字母都包含在其自己的元素中，包括单词之间的空格，因此每个字母和空格都变成一个弹性项目。
...
>对于flex容器的text-node子节点，如果text节点不为空（包含空格以外的内容），它将被包装在一个匿名的“ flex item”中，其行为就像其flex item的兄弟姐妹一样。尽管这些匿名flex项目确实继承了flex容器设置的所有flex属性，就像它们的DOM节点同级一样，但它们不能直接用CSS定位。因此，我们无法直接在其上设置任何特定于弹性商品的属性。因此，在下面的标记中，两个元素（<strong>和<em>）以及文本“它们是什么”将成为弹性项目，总共三个弹性项目：
...
>在前面的代码中，将display属性设置为flex，无序列表是flex容器，其子列表项都是flex项。这些列表项（即弹性项目）是弹性级别的框，在语义上仍会列出项目，但在其预先表示中不会列出项目。它们也不是块级框。相反，他们参与了容器的flex格式设置上下文。 “ li”元素之间和周围的空格（换行和缩进制表符和/或空格）将被完全忽略。链接本身不是弹性项目，而是列表项目已成为弹性项目的后代。
...
>伸缩项目的边距不会崩溃。 “浮动”和“清除”属性对弹性商品没有影响，也不会影响弹性商品的流动性。实际上，将“浮动”和“清除”应用于弹性项目时会被忽略。 （但是，“ float”属性仍会通过影响“ display”属性的计算值来影响盒子的生成。）请考虑：
...
>在此示例中，“ aside”是flex容器。注释和仅空白文本节点将被忽略。包含“某些文本”的文本节点被包装在一个匿名flex项目中。包含“某些文本”的标题，图像和文本节点都是弹性项目。由于图像是弹性项目，因此会忽略“浮动”。
...
>即使图像和文本节点是内联级别的节点，它们都是弹性项目，只要它们不是绝对定位的，它们就会被阻塞：
...
>在最后一个示例中，标记类似于第二个示例中的代码，并在非空文本节点内添加了链接。在这种情况下，我们将创建五个弹性项目。注释和仅空白文本节点将被忽略。标头，图像，链接之前的文本节点，链接和链接之后的文本节点都是弹性项目。如图12-39所示。
...
>包含“一些文本”和“更多文本”的文本节点被包裹在匿名flex项目中，在图12-39中由虚线框（为说明目的添加了虚线）表示，没有背景。标头，图像和链接是实际的DOM节点，可以直接使用CSS设置样式。匿名Flex容器不可直接定位，因此只能使用它们选择的任何样式
...
>另外，“ vertical-align”对伸缩项目没有任何影响，除了会影响伸缩项目中文本的对齐方式。换句话说，在flex项目上设置`vertical-align：bottom'将使flex项目内的文本全部与行框的底部对齐，而不是将flex项目推到其容器的底部。 （这就是`align-items`和`align-self`的用途。）
...
>尽管`float`实际上不会浮动flex项目，但是设置`position：absolute`是另外一个故事。就像其他任何绝对定位的元素一样，flex容器的绝对定位子元素将从文档流中删除。更重要的是，它们不参与弹性布局，也不属于文档流。但是，它们可能会受到flex容器上设置的样式的影响，就像孩子可能会受到非flex容器的父元素所影响一样。除了继承任何可继承的属性外，flex容器的属性还会影响定位的来源。
...
>flex容器的绝对定位子元素受flex容器的“ justify-content”值及其自身的“ align-self”值（如果有）的影响。例如，如果您在绝对定位的子项上设置“ align-self：center”，则该子项将从flex容器父项的横轴开始居中。从那里，它可以通过诸如top，bottom，margin等属性移动。
...
>“ order”属性（在后面的第648页中的“命令属性”一节中有解释）可能不会影响绘制绝对位置的flex容器子元素的位置，但会影响相对于其同级元素绘制子元素时的顺序。 
...
>在图12-40中，您会注意到设置为“ nowrap”默认值的行使flex容器溢出。这是因为涉及弹性商品时，“最小宽度”的隐含值为“自动”，而不是“ 0”。最初是在规范中，如果项目不适合该单一主轴，它们会收缩。但是，“最小宽度”的规范已更改为适用于弹性商品。 （传统上，“最小宽度的默认值为0”。）
...
>如果将“最小宽度”设置为比“自动”的计算值窄的宽度（例如，如果声明“最小宽度：0”），则“ nowrap”示例中的flex项将缩小为比实际内容窄（在某些情况下）。如果允许包装这些物品，则它们将尽可能缩小以适合其内容，但不会变窄。两种情况都在图12-41中进行了说明。
...
>虽然弹性项目的对齐方式，顺序和灵活性可以通过在其弹性容器上设置的属性在某种程度上进行控制，但可以将多个属性应用于单个弹性项目以进行更精细的控制。
...
>“ flex”的简写属性以及它的“ flex-grow”，“ flex-shrink”和“ flex-basis”的组成属性，控制flex项目的“ flexibility”。柔韧性是柔项沿着主轴增长或收缩的量。
...
>柔韧性布局的定义方面是使柔韧性项目“柔韧性”的能力：更改其宽度或高度以填充主要尺寸中的可用空间。伸缩容器将自由空间按其伸缩比例分配给其项目，或将其收缩以防止与伸缩比例成比例的溢出。 （我们将暂时探讨这些概念。）
...
>在弹性项目上声明`flex`速记属性，或定义组成速记的单个属性，使作者能够定义增长和收缩因子。如果有多余的空间，您可以告诉弹性项目增加以填充该空间。或不。如果没有足够的空间来以其定义的大小或默认大小将所有弹性项目容纳在弹性容器中，则可以告诉弹性项目按比例收缩以适合该空间。或不。
...
>“ flex”属性指定了弹性长度的组成部分：弹性项目的“长度”是弹性项目沿主轴的长度（请参见第579页的“理解轴”）。当盒子是柔韧性物品时，将使用“ flex”来确定盒子的尺寸，而不是主轴尺寸的维属性（“ height”或“ width”）。 “ flex”属性的“组成部分”包括“ flex增长因子”，“ flex收缩因子”和“ flex基础”。
...
>柔韧性基础决定了柔韧性增长和收缩因子的实现方式。顾名思义，flex速记的“ flex-basis”组件是flex项确定可增长多少以填充可用空间或应收缩多少以适应所有flex项的基础。足够的空间。它是每个弹性商品的初始尺寸，可以通过将增长和收缩因子都指定为0来限制为该特定尺寸：
...
>如果选择器的目标不是弹性项目，则对其应用`flex`属性将无效。
...
>“ flex-grow”属性定义了在有可用空间时是否允许弹性项目增长，如果允许增长并且有可用空间，则相对于其他弹性项目的增长，它会成比例地增长兄弟姐妹。
...
>“ flex-grow”的值始终是一个数字。负数无效。您可以根据需要使用非整数，只要它们为零或更大即可。该值设置弹性增长因子，该因子决定了在分配弹性容器的可用空间时，弹性项目相对于其余弹性项目同级将增长多少。
...
>例如，假设一个宽度为750px的水平伸缩容器，其中包含三个伸缩项目，每个伸缩项目均设置为宽度：100像素。这意味着弹性项目总共占用300个像素的空间，剩下450个像素的“剩余”或可用空间（因为750-300 = 450）。这是图12-42中所示的第一种情况。在那种情况下，任何弹性项目都不允许增长。
...
>在图12-42的第二种情况下，只有弹性项目之一（第三项）被赋予了增长因子。我们给它的声明是“ flex-grow：1”，但实际上它可以是浏览器可以理解的任何正数。在这种情况下，如果两个项目没有增长因子，而第三个项目具有增长因子，则将所有可用空间分配给具有增长因子的弹性项目。因此，第三个flex项目将所有450像素的可用空间添加到其中，最终宽度为550像素。应用于样式中其他地方的宽度：100px会被覆盖。
...
>在第三种情况和第四种情况下，尽管弯曲增长因子不同，但弯曲物品的宽度相同。让我们考虑第三种情况，增长因子分别为1、1和3。将所有因子相加在一起得出的总数为5。然后将每个因子除以该总数得出的比例。因此在这里，三个值分别除以五，得出0.2、0.2和0.6。
...
>这些是添加到每个弹性项目起始宽度100像素的增长部分。因此，最终宽度分别为190像素，190像素和370像素。
...
>第四种情况具有相同的结果，因为比例相同。想象一下，我们将生长因子更改为0.5、1和1.5。现在，数学计算得出，第一个弹性项目获得了可用空间的六分之一，第二个弹性项目获得了三分之一，第三个弹性项目得到了一半。这样一来，弹性项目的最终宽度分别为175、250和425像素。如果我们声明了0.1、0.1和0.3或25、25和75的增长因子，或者实际上是具有1：1：3对应关系的数字的任何组合，结果将是相同的。
...
>如第613页的“最小宽度”中所述，如果未设置宽度或弹性单位，则弹性单位默认为“自动”，这意味着每个弹性项目单位均为其未包装内容的宽度。 “ auto”是一个特殊值：除非项目上设置了宽度，否则默认为“ content”，此时flex-basis变为该宽度。在635页的“自动伸缩基础”中讨论了“自动”值。如果在本示例方案中，我们没有设置宽度，而字体尺寸很小，那么沿主轴线的可分配空间将超过450像素。
...
>现在，考虑一种情况，其中弹性商品具有不同的“宽度”值以及不同的增长因子。在图12-43中，在第二个示例中，我们在750像素宽的容器中具有100像素，250像素和100像素宽的弹性项目，分别具有1、1、3的增长因子。这意味着我们有300个像素的额外空间可分配到总共5个增长因子中（因为750-450 = 300）。因此，每个增长因子均为60像素（300÷5）。这意味着“ flex-grow”值为“ 1”的第一和第二flex项将分别增长60像素。最后一个flex项目将增加180像素，因为它的flex-grow值为3。
...
>回想一下图12-42中的第二个示例，其中弹性增长因子为0、0和1。因为我们仅声明了“弹性增长”的值，所以弹性基础设置为“自动”，就好像我们有声明：
...
>因此，这意味着前两个弹性项目没有增长因子，收缩因子和“自动”的弹性基础。如果我们在图12-42的示例中使用了“ flex”，而不是偶然地使用了“ flex-grow”，那么每种情况下的flex基础都将被设置为“ 0％”，就像这样做了一样：
...
>您可能会注意到在前两种情况下有些奇怪：将弹性基准设置为零，并且只有第二种情况下的最后一个弹性项目的弹性增长值为正。逻辑上，3个flex项目的宽度应该分别为0、0和750像素。但是逻辑也会指出，即使flex容器具有容纳所有内容的空间，即使将基数设置为“ 0”，让内容溢出其flex项也没有意义。
...
>规范作者想到了这个难题。当`flex`属性声明将flex-basis显式设置或默认为'0％'且flex项目的增长因子为'0'时，非增长flex项目的主轴长度将缩小到最小长度内容允许或更小。在图12-44中，最小长度是最宽字母序列“ flex：”（包括冒号）的宽度。
...
>只要flex项目有可见的溢出并且没有为min-width（或垂直主轴的min-height）明确设置值，最小的width（或最小的height）就是伸缩项目要适合内容或声明的宽度（或高度）的最小宽度（或高度），以较小者为准。
...
>如果允许所有项目增长，并且每个弹性项目的弹性基础为“ 0％”，则将根据增长因子按比例分配所有的空间，而不仅仅是多余的空间。在图12-44的第三个示例中，两个弹性项目的增长因子为1，一个弹性项目的增长因子为3。因此，我们共有五个增长因素：
...
>当默认的伸缩项目大小为100像素时，“ 0％”的伸缩基础会覆盖它，从而为我们留出2个伸缩项目，每个伸缩项分别为150像素，最后一个伸缩项目的宽度为450像素：
...
>同样，在图12-44的最后一个示例中，两个弹性项目的生长因子为0.5，一个弹性项目的生长因子为1.5，我们总共有2.5个生长因子：
...
>虽然默认的伸缩项目大小为100像素，但0％的伸缩基础会覆盖该值，从而为我们提供2个伸缩项目（每个150像素）和最后一个伸缩项目（宽度为450像素）：
...
>同样，这与仅声明“ flex-grow”不同，因为这意味着flex基础默认为“ auto”。在那种情况下，仅多余的空间而不是所有的空间按比例分配。另一方面，当使用“ flex”时，flex基础设置为“ 0％”，因此flex项目与总空间成比例增长，而不仅仅是剩余空间。差异如图12-45所示。
...
>收缩系数决定了当没有足够的空间容纳所有弹性项目时，弹性项目相对于其余弹性项目同级会收缩多少，具体取决于其内容和其他CSS属性。如果在简写的“ flex”属性值中省略，或者同时省略了“ flex”和“ flex-shrink”，则收缩系数默认为“ 1”。像增长因子一样，“ flex-shrink”的值始终是一个数字。负数无效。您可以根据需要使用非整数值，只要它们大于零即可。
...
>基本上，收缩因子定义了当弹性商品没有足够的空间并且不允许弹性容器增长或包装时，“负可用空间”的分配方式。如图12-46所示。
...
>图12-46与图12-42相似，不同之处在于flex项目被设置为`width：300px`而不是100像素。我们还有一个750像素宽的flex容器。这3个项目的总宽度为900像素，这意味着内容开始时比父Flex容器宽150像素。如果不允许物品收缩或包装（请参见第576页的“包装弯曲线”），它们将从固定尺寸的弯曲容器中破裂。这在图12-46的第一个示例中得到了证明：这些项目不会收缩，因为它们的收缩系数为零。相反，它们会使flex容器溢出。
...
>在图12-46的第二个示例中，仅将最后一个flex项目设置为能够收缩。因此，最后一个柔韧性物品被迫进行所有必要的收缩，以使所有柔韧性物品能够放入柔韧性容器中。 750像素的容器需要容纳900像素的内容，因此我们有150像素的负可用空间。没有收缩因子的2个flex项目保持300像素宽。第三个flex项目的收缩因子为正值，它会收缩150像素，最终达到150像素的宽度。这使3个项目可以放入容器中。 （在此示例中，收缩因子为“ 1”，但是如果它是“ 0.001”或“ 100”或“ 314159.65”或浏览器可以理解的任何其他正数，则结果将相同。）

In the third example, we have positive shrink factors for all three flex items:

>在第三个示例中，我们对所有三个弹性项目都有正的收缩因子：
...
>如果允许所有项目都缩小（如此处的情况），则根据缩小因子按比例分配缩小。这意味着弹性项目的收缩因子越大，与其同级弹性项目的收缩因子相比，该项目的收缩因子就越大。
...
>父级宽度为750像素，而3个弹性项目的宽度为300像素，则需要刮除150个“负空间”像素，以去除允许收缩的弹性项目（在此示例中全部包含） 。对于两个伸缩系数为1的弹性项目，以及一个伸缩系数为3的弹性项目，我们总共有五个伸缩系数：
...
>默认的伸缩项目大小为300像素，这使我们拥有2个伸缩项目，每个伸缩项目的宽度为270像素，最后一个伸缩项目的宽度为210像素，总计750像素：
...
>以下CSS产生相同的结果：虽然收缩系数的数字表示形式不同，但它们成比例地相同，因此弹性项目宽度将相同：
...
>请注意，这些示例中的弹性项目将分别缩小到210、210和270像素，只要每个弹性项目中的内容（如媒体对象或不可包装的文本）不宽于210、210或270像素。如果弹性项目包含无法在主维度中包装或收缩的内容，则弹性项目将不会进一步收缩。
...
>假设第一个flex项目包含一个300像素宽的图像。该第一个伸缩项目不能收缩，其他伸缩项目可以收缩，因此它不会收缩，就好像它的收缩因子为空。在这种情况下，第一项将为300像素，负像素空间的150像素将根据第二和第三弯曲项的收缩因子成比例地分布。
...
>在这种情况下，对于150像素的负空间，我们有4个不受阻碍的收缩因子（一个来自第二个flex项目，三个来自三个flex项目），每个收缩因子值37.5像素。 flex项目将分别结束300、262.5和187.5像素，总共750像素。结果如图12-47所示：
...
>如果图像的宽度为296像素，则第一个flex项目将能够缩小4像素。然后，将剩余的146个负空间像素分配给剩余的4个缩小区域，每个系数产生36.5个像素。然后，弹性项目的宽度分别为296、263.5和190.5像素。
...
>如果所有三个flex项目都包含不可折叠的文本或300像素或更宽的媒体，则三个flex项目都不会缩小，这类似于图12-46中的第一个示例。
...
>前面的代码示例非常简单，因为所有flex项目均以相同的宽度开头。但是如果宽度不同怎么办？如果第一个和最后一个弹性项目的宽度为250像素，中间的弹性项目的宽度为500像素，如图12-48所示，该怎么办？
...
>弹性项目相对于“收缩因子”和弹性项目的宽度均按比例缩小，宽度通常是弹性项目内容的宽度，没有包裹。在图12-48中，我们尝试将1,000个像素放入750像素宽度的flex容器中。我们需要从5个缩小因子中删除250个像素。
...
>在这里，我们有250个像素的负空间按比例分布。为了获得收缩因子的比例，我们将负值空间除以弹性项目宽度的总和（更确切地说，是它们沿主轴的长度）乘以其收缩因子：
...
>当我们将每个flex项目减少“ flex-shrink”值的16.67％时，最终得到的flex项目减少了：
...
>然后，分别从250、500和250像素的起始大小中减去每个缩小值。因此，我们最终得到了208.33、416.67和125像素宽的flex项目。
...
>如果收缩因子为零，则如果将弹性项目的宽度和弹性基础都设置为“自动”，则即使您认为适当，其内容也不会包装。相反，任何正收缩值都可以使内容包装。因为收缩是基于收缩因子成比例的，所以如果所有弹性项目都具有相似的收缩因子，则内容应换行相似的行数。
...
>在图12-49所示的三个示例中，弹性项目没有声明的宽度。因此，宽度基于内容，因为“宽度”默认为“自动”。 flex容器的宽度已设为520像素，而不是我们通常的750像素。
...
>请注意，在第一个示例中，所有项目均具有相同的“ flex-shrink”值，所有内容均环绕四行。在第二个示例中，第一个弹性项目的收缩因子是其他弹性项目的一半，因此它将内容包装在（大约）行数的一半上。这就是收缩因子的力量。
...
>因为`flex`属性的收缩因子会按比例减小弹性项目的宽度，所以弹性项目中的文本行数会随着宽度的缩小或增长而增加或收缩，从而导致同级弹性项目中的相似高度内容相似。
...
>在示例中，将flex项目的内容分别设置为280、995和480像素-这是第三个示例中非包装flex项目的宽度（由开发人员工具测量，然后四舍五入为这个例子更简单）。这意味着我们必须根据其收缩因子按比例缩小弹性项目，以将1755像素的内容装入520像素宽的弹性容器中。这意味着我们有1,235像素的负可用空间要按比例分配。
...
>在我们的第一个示例中，flex项将以相同或近似相同的文本行数结束。这是因为弹性项目根据其内容的宽度成比例地缩小。
...
>我们没有声明任何宽度，因此不能像前面的示例一样简单地使用显式元素宽度作为计算的基础。相反，我们根据内容的宽度（分别为280、995和480像素）的宽度按比例分配负空间的1,235像素。我们确定520是1,755的29.63％。要确定收缩系数为1的每个弹性项目的宽度，我们将每个弹性项目的内容宽度乘以29.63％：
...
>默认使用“ align-items：Stretch”（对齐项目：拉伸）（请参见第596页的“对齐项目”），三列布局将具有三列等高的列。通过对所有弹性项目使用一致的收缩因子，可以指示这三个弹性项目的实际内容应该具有近似相等的高度，尽管这样做，这些列的宽度不一定是一致的。
...
>在图12-49的第二个示例中，弹性项目的收缩率并不相同。第一个伸缩项目将按比例缩小一半。我们以相同的宽度开始：分别为280、995和480像素，但它们的收缩因子分别为0.5、1.0和1.0。我们知道内容的宽度，可以从数学上找到收缩率（X）：
...
>加上各种不同的收缩和增长因素，使其变得不太直观。这就是为什么您可能希望始终声明flex简写的原因，最好为每个flex项目设置宽度或基数。如果这还没有意义，请不要担心；我们将在讨论“伸缩基础”时再介绍一些缩小的示例。
...
>像这样允许弹性项目按比例收缩，可以使响应对象和布局按比例收缩而不会破裂。
...
>这意味着它们不会从基础上增长，但是它们可以以相等的速度收缩。这意味着默认情况下，它们将具有其伸缩基础的宽度。如果确实需要缩小，则将其缩小到最小宽度“ 150px”，然后停止缩小。但是，如果其中一个元素的宽度超过150像素（无论是图像还是文本），则一旦达到该内容宽度，它就会停止缩小。假设将一个180像素的图像放入了“ aside”元素中。一旦达到180像素宽，它将停止缩小。 nav将继续缩小到150像素。
...
>因此，如果有足够的空间可以容纳`main`元素，那么它可以增长。由于它是唯一可以增长的弹性项目，因此可以实现所有增长。这意味着，如果浏览器窗口的宽度为1300像素，则两个侧栏的宽度将分别为200像素，中间栏的宽度为900像素。在缩小的情况下，中心列的缩小速度是其他两个元素的两倍。因此，如果浏览器窗口的宽度为900像素，则侧栏的宽度均为175像素，中间栏的宽度为550像素。
...
>请注意，在这种情况下不需要使用像素。您甚至不必为各种弹性底座使用相同的单位度量。前面的示例可以这样重写：
...
>正如我们已经看到的，弹性项目的大小受其内容和框模型属性的影响，可以通过“弹性”属性的三个组成部分进行重置。 “ flex”属性的“ <flexbasis>”组件定义了弹性项目的初始大小或默认大小，然后才分配额外的空间或负的空间-在允许弹性项目根据增长和收缩因素进行增长或收缩之前。也可以通过flex-basis属性设置。
...
>弹性基础决定弹性项目的元素框的大小，该尺寸由“ box-sizing”设置。默认情况下，当块级元素不是弹性项目时，其大小由其父项，内容和框模型属性的大小确定。如果没有显式声明或继承任何大小属性，则大小默认为其单独的内容，边框和填充，这是块级元素父大小的100％。
...
>在完成支持之前，可以轻松地填充“ flex-basis：content;”，因为这相当于声明“ flex-basis：auto;”。宽度：自动；在该弹性项目上，或“弹性基础：自动； height：自动；如果主尺寸是垂直的，则为。不幸的是，在不支持的浏览器中使用`flex`速记中的`content`会使整个声明无效（请参见第579页的“理解轴”）。
...
>在图12-52的第一个和第三个示例中，flex项的宽度是内容的大小；而弹性的基础是相同的尺寸。在第一个示例中，弹性项目的宽度和基础约为132像素。并排放置的3个flex项目的总宽度为396像素，可轻松装入父容器。
...
>在第三个示例中，我们设置了一个空的收缩因子（`0`）：这意味着flex项无法收缩，因此它们不会收缩或包装以适合固定宽度的flex容器。相反，它们是未包装文本的宽度。该宽度也是flex基础的值。伸缩项目的宽度和基础分别约为309、1,037像素和523像素。您根本看不到第二个弹性项目或第三个弹性项目的完整宽度，但它们在章节文件中。
...
>第二个示例包含与第三个示例相同的内容，但是flex项的默认缩放比例为1，因此此示例中的文本自动换行，因为flex项可以收缩。因此，弹性项目的宽度不是内容的宽度，而弹性基础（将按比例缩小的基础）是项目内容的宽度。
...
>当设置为“ auto”时，无论是显式还是默认设置，“ flex-basis”与元素的主轴尺寸相同，前提是该元素未转换为flex项目。对于长度值，flex-basis解析为`width`或`height`值，不同之处在于，当`width`或`height`的值为auto时，flex基础值回落为`content'。 。
...
>当没有其他属性设置弹性项目的主轴尺寸时（即，在这些弹性项目上没有设置“宽度”甚至“最小宽度”），以及“ flex-basis：auto”或“ flex” ：0 1 auto`被设置，伸缩项目将仅与内容适合所需的宽度一样，如图12-53中的第一个示例所示。在这种情况下，它们是文本“ flex-basis：auto;”的宽度，大约为110像素。弹性项目是其预先弯曲的大小，就像设置为“ display：inline-block”一样。在此示例中，它们是在主启动时分组的，因为flex容器的“ justify-content”默认为“ flex-start”。
...
>在图12-53的第二个示例中，每个flex项目都具有“ auto”的flex基础和一个显式声明的宽度。如果不将元素变成柔性项目，则元素的主轴尺寸将分别为100、150和200像素。因此它们就在这里，因为它们可以装入flex容器，而沿主轴没有任何溢出。
...
>在图12-53的第三个示例中，每个flex项目都具有“ auto”的flex基础和一个非常大的显式声明的宽度。如果不将元素转换为弹性项目，元素的主轴尺寸将分别为2,000、3,000和4,000像素。由于它们不可能沿着主轴溢出而无法装入flex容器中，并且它们的flex收缩因子都默认为1，因此它们会收缩直到适合flex容器。您可以使用上一节中概述的过程进行数学运算以找出它们的大小。作为提示，第三个弹性项目应从4000像素减少到240像素的宽度。
...
>如果既未设置“ flex-basis”又未设置“ flex”，则flex项目的主轴尺寸为该项目的preflex尺寸，因为它们的默认值为“ auto”。
...
>在图12-54中，发生了两件事：flex base默认为auto，增长因子默认为0，每一项的收缩因子默认为1。对于每个伸缩项目，伸缩基础是其各自的“宽度”值。这意味着将flex base设置为“ width”属性的值：在第一个示例中为100、200和300像素，在第二个示例中为200、400和200像素。由于弹性项目的组合宽度分别为600像素和800像素，两者均大于540像素宽的容器的主轴尺寸，因此它们都按比例缩小以适合。
...
>在第一个示例中，我们试图在540个像素中容纳600个像素，因此每个flex项目将缩小10％，以产生90、180和270像素宽的flex项目。在第二个示例中，我们尝试将800像素调整为540像素，以使它们全部缩小32.5％，从而使flex项目的宽度分别为135、270和135像素。
...
>在前面的示例中，`auto` flex base默认为各种flex项目声明的宽度。还有其他选择；例如，我们可以为flex-basis值使用与“ width”和“ height”相同的长度单位。
...
>如果同时具有“ flex-basis”和“ width”（或垂直轴的“ height”）值，则基数优先于宽度（或高度）。让我们将基值添加到图12-54中的第一个示例中。弹性项目包括以下CSS：
...
>宽度被基数覆盖。伸缩项目分别缩小到270像素，180像素和90像素。如果容器“ not”的宽度受限制，则弹性项目分别为300像素，200像素和100像素。
...
>虽然声明的flex基础可以覆盖flex项目的主轴尺寸，但是尺寸可能会受到其他属性的影响，例如“最小宽度”，“最小高度”，“最大宽度”和“最大高度” `。这些不容忽视。因此，例如，一个元素可能具有“ flex-basis：100px”和“ min-width：500px”。即使flex基础较小，也要遵守最小宽度“ 500px”。
...
>在图12-56的第二个示例中，前两个弹性项目的弹性基础为“自动”，默认“宽度”为“自动”，就好像其弹性基础设置为“内容”一样。
...
>百分比值相对于父像素为540像素。以'100％'为基础的第三个flex项目并不是非包装flex容器中的唯一flex项目。因此，它不会增长到父级伸缩容器宽度的100％，除非它的收缩因子设置为空收缩因子，这意味着它不能收缩，或者包含不可收缩的内容，例如比父容器宽或宽。
...
>每个弹性项目将收缩28.95％，变成其宽度（如果不允许收缩）的71.05％。我们可以算出最终的宽度：
...
>只要伸缩项可以这么小，这些数字就成立。也就是说，只要所有flex项目都不包含宽度超过78.16像素或383.68像素的媒体或不间断文本。只要内容可以包装为该宽度或更窄，这就是这些flex项目的最大宽度。之所以说“最宽”，是因为如果其他两个弹性项目之一不能缩小到该值那么窄，则它们将不得不吸收一些负空间。
...
>我们宣布3个flex项目的“ flex-basis”分别为“ 70％”，“ auto”和“ 80％”。请记住，在我们的场景中，“ auto”是非包装内容的宽度，在这种情况下约为110像素，而我们的flex容器为540像素，基数等于：
...
>当我们添加这3个flex项目的基础的宽度时，它们的总合并宽度为920像素，这需要适合540像素宽的flex容器。因此，我们有380个负空间像素，可以在3个flex项目之间按比例删除。为了弄清楚比率，我们将伸缩容器的可用宽度除以伸缩项目无法收缩时所具有的伸缩项目的宽度之和：
...
>因为收缩因子都相同，所以这很简单。如果没有弹性项目同级，则每个项目的宽度将是宽度的58.7％：
...
>如果容器的宽度不同，会发生什么？说1,000个像素？弹性基础分别为700像素（70％×1,000像素），110像素和800像素（80％×1,000像素），总共1,610像素：
...
>因为使用70％和80％的基数，flex项目的合并基数将始终大于100％，无论我们将父级设置为多大，这3个项目都将始终缩小。
...
>如果第一个flex项由于某种原因而不能收缩（无论是由于内容不可收缩，还是由于CSS的另一部分将其“ flex-shrink”设置为“ 0”），则将是父级宽度的70％（378个像素）在这种情况下。其他2个flex项目必须按比例缩小以适合剩余的30％或162像素。在这种情况下，我们希望宽度为378像素，32.875像素和129.125像素。由于文本“ basis：”比其宽（假设为42像素），我们得到378像素，42像素和120像素。结果如图12-57所示。
...
>在“ flex-basis：auto”的情况下，基础是flex项目内容的主要尺寸。如果每个flex项目的基础是“ 0”，则“可用”空间是整个flex容器的主轴尺寸。无论哪种情况，“可用”空间都是根据每个弹性项目的增长因素成比例分配的。
...
>在基数为“ 0”的情况下，将弹性容器的大小进行划分，并根据它们的增长因子按比例分配给每个弹性项目-它们的默认原始主轴大小由“高度”，“宽度”定义尽管“最小宽度”，“最大宽度”，“最小高度”和“最大高度”确实会影响弹性尺寸，但并未考虑“内容”。
...
>如图12-58所示，当基础是“自动”时，只是多余的空间按比例分配并添加到每个可伸缩的伸缩项目集。同样，假设文本“ flex：X X auto”的宽度为110像素，在第一个示例中，我们有210个像素可分布在6个增长因子中，或每个增长因子35个像素。伸缩项目分别为180、145和215像素宽。
...
>在第二个示例中，当基数为0时，宽度的所有540像素都是可分配空间。 6个增长因子之间有540像素可分配空间，每个增长因子值90像素。伸缩项目的宽度分别为180、90和270像素。
...
>尽管中间的flex项宽为90像素，但此示例中的内容比110像素窄，因此该flex项没有包装。
...
>既然我们对构成`flex`速记的属性有了更全面的了解，请记住：始终使用`flex`速记。它接受通常的全局属性值，包括“ initial”，“ auto”，“ none”；以及通常为1的整数的使用，表示flex项目可以增长。让我们来看看所有这些值。
...
>该值根据主轴的方向，根据“ width”或“ height”属性来调整弹性项目的尺寸，同时允许收缩。
...
>这个flex值也基于`width`或`height`属性来调整flex项目的大小，但是使它们完全灵活，既可以收缩也可以增长。
...
>该值再次根据“ width”或“ height”属性调整弹性商品的大小，但使它们完全不灵活：它们无法收缩或增长。
...
>此值将弹性项目的增长因子设置为提供的“ <数字>”。因此，它将收缩因子设置为“ 0”，将基数也设置为“ 0”。这意味着`width`或`height`值是最小尺寸，但是flex项目会在有空间的情况下增长。
...
>声明“ flex：initial”将设置空增长因子，将收缩因子设置为“ 1”，并将flex base设置为“ auto”。在图12-59中，我们可以看到`auto` flex base的效果。在前两个示例中，每个弹性项目的基础都是“内容”，每个弹性项目的宽度都是组成其内容的单行字母的宽度。在最后两个示例中，所有项目的flex基数都等于50像素，因为已将'width：50px'应用于所有flex项目。 “ flex：initial”声明将“ flex-basis”设置为“ auto”，我们之前看到的是“ width”（或“ height”）（如果声明）或“ content”（如果未声明）的值。
...
>在图12-59的第一个和第三个示例中，我们看到当flex容器太小而无法以其默认主轴大小容纳所有flex项目时，flex项目会收缩，以使所有flex项目都适合父对象伸缩容器。在这些示例中，所有柔韧性项目的组合柔韧性基础大于柔韧性容器的主轴尺寸。在第一个示例中，示教柔性项目的宽度根据每个项目内容的宽度及其收缩能力而变化。它们均基于其收缩因子按比例收缩，但不比其最宽的内容窄。在第三个示例中，每个flex项的flex-basis为50像素（由于`width`的值），所有项均等收缩。
...
>默认情况下，Flex项在主开始处进行分组，因为“ flex-start”是“ justify-content”属性的默认值。这仅在柔性线中的柔性项目的组合主轴尺寸小于柔性容器的主轴尺寸且所有柔性项目均无法增长时才注意到。
...
>`flex：auto`与`flex：initial`类似，但是使flex项在两个方向上都具有灵活性：如果没有足够的空间容纳容器中的所有项，它们会收缩，并且它们会成长为如果有可分配的空间，请占用容器中所有多余的空间。伸缩项目会吸收主轴上的所有自由空间。以下两个语句是等效的：
...
>图12-60的第一个和第三个示例与图12-59的示例相同，因为收缩率和基数相同。但是，第二和第四示例是不同的。这是因为设置了“ flex：auto”时，增长因子为“ 1”，因此flex项目可以增长以合并所有额外可用空间。
...
>任何“ flex：none” flex项目都是不灵活的：它们既不能收缩也不能增长。以下两行CSS是等效的：
...
>如图12-61的第一个和第三个示例所示，如果没有足够的空间，则伸缩项会溢出伸缩容器。这与“ flex：initial”和“ flex：auto”不同，它们都设置了正的收缩系数。
...
>基础解析为“自动”，这意味着每个弹性项目的主轴尺寸由元素的主轴尺寸确定，前提是该元素未转换为弹性项目。 flex-basis解析为元素的width或height值。如果该值为“ auto”，则基础将成为内容的主轴尺寸。在前两个示例中，基础（以及宽度，因为没有增长或缩小）是内容的宽度。在第三个和第四个示例中，width和base均为50像素，因为这是应用于它们的`width`属性的值。
...
>这使在其上设置的flex项目变得灵活：它可以增长。收缩因子实际上是没有意义的：弹性基础设置为0，因此弹性项目只能在该基础上增长。
...
>在图12-62的前两个示例中，所有弹性项目的弹性增长因子均为3。弹性基数为“ 0”，因此它们不会“缩小”；它们只是从零像素宽开始平均增长，直到它们的主轴大小之和增长到沿着主轴填充容器为止。在所有flex项目都以'0'为基准的情况下，主要维度的100％是可分配空间。在第二个示例中，柔件的主轴尺寸更宽，因为较宽的柔件容器具有更多的可分配空间。
...
>任何大于0甚至0.1的数值都表示弹性项目可以增长。当有可用空间增长时，如果只有一个弹性项目具有正增长因子，则该项目将占用所有可用空间。如果有多个弹性项目可以增长，则可用的额外空间将根据它们的增长因子按比例分配给每个弹性项目。
...
>在图12-62的最后三个示例中，有六个flex项目，分别是`flex：0`，`flex：1`，`flex：2`，`flex：3`，`flex：4`和`flex ：分别声明了5`。这些是弹性商品的增长因子，每一个的收缩因子均为1，弹性基础为0。每个的主轴尺寸与指定的挠曲增长因子成比例。您可能会假设第三和第四示例中带有文本“ flex：0”的“ flex：0”项的宽度为零像素，就像第四和第五示例中一样，但是默认情况下，flex项不会缩小小于最长字或固定大小元素的长度。
...
>在图中添加了一些填充，边距和边框，以使视觉效果更令人愉悦。因此，最左侧的flex项目（声明为flex：0）是可见的：即使边框的宽度为零像素，它的边框也只有一个像素，因此可见。
...
>默认情况下，弹性项目的显示和布局顺序与它们在源代码中的显示顺序相同。弹性项目和弹性线的顺序可以通过“ flex-direction”颠倒，但是有时您需要更复杂的重排。 “ order”属性可用于更改各个弹性项目的顺序。
...
>默认情况下，所有弹性项目均被分配为“ 0”的顺序，所有弹性项目均被分配给相同的序数组，并沿其主轴方向按与它们的源顺序相同的顺序显示。 （本章中看到的所有示例都是这种情况。）
...
>要更改弹性项目的视觉顺序，请将`order`属性值设置为非零整数。在不是flex容器的子元素的元素上设置`order`属性不会对此类元素产生影响。
...
>“ order”属性的值指定弹性项目所属的序数组。绘制到页面上时，任何具有负值的弹性项目都将出现在默认值“ 0”之前，而所有具有正值的弹性项目将出现在默认值为“ 0”之后。在视觉上进行更改后，源顺序保持不变。屏幕阅读器和制表顺序保持不变，如HTML的源顺序所定义。
...
>第七个弹性项目是第一个被布置的项目，这是由于`order`属性的负值，它小于默认的'0'，并且也是其所有同级弹性项目中的最低值。第六个弹性项目是唯一一个值大于零的项目，因此在所有同级项中具有最高的订单值。这就是为什么将其放置在所有其他弹性项目之后的原因。所有其他项目的默认“顺序”均为“ 0”，它们以与源顺序相同的顺序绘制在这些第一项和最后一项之间，因为它们都是同一序数组（“ 0”）的成员。
...
>flex容器按顺序修改后的文档顺序排列其内容，从编号最低的顺序组开始，然后向上。当多个弹性商品的order属性值相同时，这些商品共享一个序数组。每个顺序组中的项目将按源顺序显示，并且该组按从最低到最高的数字顺序显示。考虑以下：
...
>通过为多个弹性项目设置相同的“订单”值，这些商品将按序数组显示，并按每个序数组中的源顺序显示。结果如图12-64所示。
...
>这种重新排序纯粹是视觉上的。屏幕阅读器“应该”阅读文档在源代码中显示的内容，尽管可能没有。作为一种视觉上的变化，伸缩项目的排序会影响页面的绘制顺序：伸缩项目的绘制顺序是它们显示的顺序，就好像它们在源文档中已被重新排序一样，即使它们没有。
...
>当前活动的选项卡添加了.active类，删除了href属性，并将order设置为-1，这比其他同级flex项目的默认值0少，这意味着首先出现。
...
>通过使用order：-1声明，我们可以使nav首先出现，因为它是-1序数组中唯一的flex项目。没有明确声明“订单”的“商品”和“旁边”，默认为“订单：0”。
...
>请记住，当同一序数组中有多个弹性项目时，该组中的成员将按照源顺序从主开始到维护的方向显示，因此，“文章”显示在“旁边”之前。
...
>一些开发人员在更改至少一个弹性项目的顺序时，喜欢为所有弹性项目赋予“ order”值，以提高标记的可读性。我们还可以写：
